...
class VirtualPinTile extends StatelessWidget {
    ...
    return PremiumPanel(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
                  ...
                  child: Row(
                    ...
                    children: [
                      Switch(
                        value: active,
                        onChanged: (value) {
                          FirestoreService.togglePinState(pin.id, value);
                        },
                      ),
          ...
          return SizedBox(width: 320, child: tile);
        },
      ),
    );
  }
  ...
  Future<void> _handleMenu(BuildContext context, String action) async {
    switch (action) {
      case 'edit_label':
        final label = await _showEditDialog(context, 'Edit Label', pin.label);
        if (label != null && label.trim().isNotEmpty) {
          await FirestoreService.updatePin(pin.id, {'label': label.trim()});
      ...
      case 'edit_pin':
        ...
        final pinNumber = int.tryParse(value ?? '');
        if (pinNumber == null) {
          showErrorToast('Invalid pin number.');
          return;
        }
        await FirestoreService.updatePin(pin.id, {'pin': pinNumber});
